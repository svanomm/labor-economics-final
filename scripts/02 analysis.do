...
outreg2 using "results", replace bd(3) sd(3) label dta adjr2 ctitle("Basic OLS") addtext("Region FE","No")
...
outreg2 using "results", append bd(3) sd(3) label dta adjr2 ctitle("OLS w/ Controls") addtext("Region FE","Yes")
}
...
drop if inrange(_n, 22, 37) | inlist(_n, 1, 3, 40)
drop v7 v8 v10-v16

drop if regexm(v1[_n+1], "Census Region")
drop if regexm(v1, "Census Region")

replace v1 = "Clustered standard errors in parentheses" in 19
replace v1 = "" in 1

texsave using "results.tex", title("Regression Results") autonumber hlines(1 15) replace
...
set obs 8
replace v1 = "Clustered standard errors in parentheses" in 7
replace v1 = "*** p<0.01, ** p<0.05, * p<0.1" in 8
ren (v1 v2 v3) (Model Coefficient SE)
texsave using "results summary.tex", title("Summary of Estimated Wage Effects") hlines(6) replace
}
...
drop v3 v5
drop if inrange(_n, 4, 22) | inlist(_n, 1, 33)
replace v1 = "Clustered standard errors in parentheses" in 16
replace v1 = "" in 2

texsave using "iv stage 1.tex", title("IV Regression Stage 1 Results") autonumber hlines(2 12) replace

* stage 2
u "iv results_dta", clear

drop v2 v4
drop if inrange(_n, 23, 30) | inlist(_n, 1, 4, 33)
drop if regexm(v1[_n+1], "Census Region")
drop if regexm(v1, "Census Region")
replace v1 = "Clustered standard errors in parentheses" in 20
replace v1 = "" in 2

texsave using "iv stage 2.tex", title("IV Regression Stage 2 Results") autonumber hlines(2 16) replace
}
...
drop v2 v4
keep if !mi(v3) | inlist(_n, 36, 37)
drop in 1
replace v1 = "Clustered standard errors in parentheses" in 17
replace v1 = "" in 1
replace v3 = "Work Status" in 1
replace v3 = "No" if v1 == "Region FE"

texsave using "heckman stage 1.tex", title("Heckman Selection Stage 1 Results") autonumber hlines(1 13) replace

* stage 2
u "heckman_dta", clear	

drop v3 v4
keep if !mi(v2) | inlist(_n, 36, 37)
drop in 1
drop if regexm(v1[_n+1], "Census Region")
drop if regexm(v1, "Census Region")
replace v1 = "Clustered standard errors in parentheses" in 19
...
texsave using "heckman stage 2.tex", title("Heckman Selection Stage 2 Results") autonumber hlines(1 15) replace
}
